...
HINSTANCE g_hInst;
LPSTR lpszClass="OVG PG: Sample 07_02";
...
int AppDisplay( void )
{
    VGImage image, son, grandson;

    VGfloat clearColor[] = {1.0f, 1.0f, 1.0f, 1.0f};
    vgSetfv(VG_CLEAR_COLOR, 4, clearColor);
    vgClear(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);

    image = vgCreateImage(VG_sRGBA_8888, 120, 120, VG_IMAGE_QUALITY_NONANTIALIASED );
    vgImageSubData(image, cimg_flower, 240*4, VG_sRGBA_8888, 0, 0, 120, 120);
    son = vgCreateImage(VG_sRGBA_8888, 60, 60, VG_IMAGE_QUALITY_NONANTIALIASED );
    son = vgChildImage(image, 60,60,60,60);
    grandson = vgCreateImage(VG_sRGBA_8888, 30, 30, VG_IMAGE_QUALITY_NONANTIALIASED );
    grandson = vgChildImage(son, 30,30,30,30);
    ...
    vgDrawImage(son);
    vgTranslate(60,0);
    vgDrawImage(grandson);

    clearColor[0] = clearColor[1] = 0.0;
    vgSetfv(VG_CLEAR_COLOR, 4, clearColor);
    vgClearImage(grandson, 15,15,15,15);
