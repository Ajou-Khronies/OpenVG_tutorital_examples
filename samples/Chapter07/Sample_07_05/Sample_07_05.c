...
HINSTANCE g_hInst;
LPSTR lpszClass="OVG PG: Sample 07_05";
...
int AppDisplay( void )
{
    ...
    const VGshort kernel[] = { 0, 0, -1, 0, 0,
                               0, -1, -2, -1, 0,
                              -1, -2, 16, -2, -1,
                               0, -1, -2, -1, 0,
                               0, 0, -1, 0, 0 }; // Laplacian of Gaussian
    vgSetfv(VG_CLEAR_COLOR, 4, clearColor);
    vgClear(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);

    src = vgCreateImage(VG_sRGBA_8888, 120, 120, VG_IMAGE_QUALITY_NONANTIALIASED );
    dst = vgCreateImage(VG_sRGBA_8888, 120, 120, VG_IMAGE_QUALITY_NONANTIALIASED );
    vgImageSubData(src, cimg_flower, 240*4, VG_sRGBA_8888, 0, 0, 120, 120);

    vgSeti(VG_MATRIX_MODE, VG_MATRIX_IMAGE_USER_TO_SURFACE);
    vgTranslate(60,30);
    vgDrawImage(src);
    clearColor[0] = clearColor[1] = clearColor[2] = 0.0;
    vgClearImage(dst, 0,0,120,120);
    vgSeti(VG_FILTER_CHANNEL_MASK, (VG_RED));
    vgConvolve(dst, src, 5, 5, 2, 2, kernel, (1.0), 0.0, VG_TILE_FILL);
